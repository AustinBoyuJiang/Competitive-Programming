...
VI num;
inline void get(int u,int pos,int cnt){
	if(pos==5){
		if(cnt){
			if(cnt&1) ans+=flag[num];
			else ans-=flag[num];
		}
		return;
	}
...
void solve(int Case){
	read(n);
	rep(i,1,n){
		rep(j,1,5) read(a[i][j]);
		sort(a[i]+1,a[i]+6);
		get(i,0,0);
		add(i,0);
	}
	write(n*(n-1)/2-ans,endl);
}
