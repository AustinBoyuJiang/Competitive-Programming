...
inline void SOLVE(int Case){
	...
	int ans=0;
	rep(i,1,n){
		if(a[i+1].fir!=a[i].fir+1||i==n){
			chkmax(ans,min(m/a[i].fir,a[i].sec)*a[i].fir);
			continue;
		}
		int need=min(m/a[i].fir,a[i].sec);
		int used=min((m-a[i].fir*need)/(a[i].fir+1),a[i+1].sec);
		int res=a[i].fir*need+used*(a[i].fir+1);
		chkmax(ans,min(m-res,min(a[i+1].sec-used,need))+res);
	}
	cout<<ans<<endl;
}
