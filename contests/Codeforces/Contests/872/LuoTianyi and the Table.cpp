...
void SOLVE(int Case){
	...
	sort(a+1,a+n*m+1);
	int ans=-INF;
	chkmax(ans,(n-1)*m*(a[n*m]-a[1])+(m-1)*(a[n*m]-a[2]));
	chkmax(ans,n*(m-1)*(a[n*m]-a[1])+(n-1)*(a[n*m]-a[2]));
	
	chkmax(ans,(n-1)*m*(a[n*m]-a[2])+(m-1)*(a[n*m]-a[1]));
	chkmax(ans,n*(m-1)*(a[n*m]-a[2])+(n-1)*(a[n*m]-a[1]));
	
	chkmax(ans,(n-1)*m*(a[n*m-1]-a[1])+(m-1)*(a[n*m]-a[1]));
	chkmax(ans,n*(m-1)*(a[n*m-1]-a[1])+(n-1)*(a[n*m]-a[1]));
	
	chkmax(ans,(n-1)*m*(a[n*m]-a[1])+(m-1)*(a[n*m-1]-a[1]));
	chkmax(ans,n*(m-1)*(a[n*m]-a[1])+(n-1)*(a[n*m-1]-a[1]));
