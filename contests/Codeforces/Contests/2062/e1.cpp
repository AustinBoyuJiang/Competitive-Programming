...
int n,a[N],father[N],mx[N],cmx[N];
VI e[N];

void check(int &mx,int &cmx,int x){
	if(x>mx){
		cmx=mx;
		mx=x;
	}
...
void init(int u,int fa){
	...
	for(int v:e[u]){
		if(v==fa) continue;
		init(v,u);
		check(mx[u],cmx[u],mx[v]);
		check(mx[u],cmx[u],cmx[v]);
	}
}

int find(int u,int fa,int tar){
	if(a[u]==tar) return u; 
	for(int v:e[u]){
		if(v==fa) continue;
		int res=find(v,u,tar);
		if(res) return res;
	}
	return 0;
}
...
int dfs(int u,int fa){
	if(mx[u]==a[u]){
		int res=find(u,fa,cmx[u]);
//		cout<<u<<" "<<res<<endl;
		return res;
	}
	if(fa==0&&e[u].size()==1&&a[e[u][0]]<a[u]){
		return e[u][0];
	}
	else if(e[u].size()==1&&a[e[u][0]^e[u][1]^fa]<a[u]){
		int res=e[u][0]^e[u][1]^fa;
		return res;
	}
	int cnt=0,mxx=mx[u],tar=-INF,hson=0;
	for(int v:e[u]){
		if(v==fa) continue;
		if(mx[v]==mx[u]){
			cnt++;
...
//	cout<<u<<" "<<tar<<" "<<cmx[hson]<<endl;
	if(cnt>1){
		...
		if(tar>=cmx[hson]){
			...
			int nxt=find(hson,u,nxt);
			return dfs(nxt,father[nxt]);
		}
	}
	return 0;
}
