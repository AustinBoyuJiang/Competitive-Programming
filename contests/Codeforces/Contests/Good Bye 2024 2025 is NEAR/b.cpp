...
inline void SOLVE(int Case){
	...
	rep(i,1,n*2){
		flag[i]+=flag[i-1]+(cnt[i]>0?1:0);
	}
	rep(i,1,n){
//		cout<<flag[a[i].sec]-flag[a[i].fir-1]<<" "<<a[i].sec-a[i].fir+1<<endl;
		if(a[i].fir==a[i].sec&&cnt[a[i].fir]==1){
			cout<<1;
			continue;
		}
		if(flag[a[i].sec]-flag[a[i].fir-1]==a[i].sec-a[i].fir+1){
			cout<<0;
		}
		else{
			cout<<1;
