...
void SOLVE(int Case){
	...
	rep(i,1,n-k+1) chkmax(ans,sum[i+k-1]-sum[i-1]+r[i+k]+l[i-1]);
	rep(l,1,n) rep(r,l,min(l+k-1,n)) chkmax(ans,sum[r]-sum[l-1]);
	rep(j,0,k){
		mn=0;
		rep(i,1,n){
			sum[i]=sum[i-1]+a[i];
			if(i<=j) sum[i]+=x;
			else if(i>n-(k-j)) sum[i]+=x;
			else sum[i]-=x;
			chkmin(mn,sum[i]);
			chkmax(ans,sum[i]-mn);
		}
