...
int n,k,w[N],c[N],siz[N];
ll ans; 
VI e[N];
unordered_map<int,bool> vis[N];

void clear(unordered_map<int,bool> &v){
	v=unordered_map<int,bool>();
}
...
void dfs1(int u,int fa){
	...
	for(int v:e[u]){
		...
		dfs1(v,u);
		siz[u]+=siz[v];
		if(hson==0||siz[v]>siz[hson]) hson=v;
	}
	if(hson){
		swap(vis[u],vis[hson]);
	}
