...
int n,m,fact[N],vis[N][3];
VI e[N];

int dfs(int u,int fa,int p){
	if(vis[u][p]!=-1) return vis[u][p];
	int cnt1=0,cnt2=0;
	VI vv;
	for(int v:e[u]){
		if(v==fa) continue;
	...
	if(vv.size()==0){
		return vis[u][p]=fact[cnt1];
	}
	if(vv.size()==1){
		if(p==0){
			vis[u][p]=(dfs(vv[0],u,1)+dfs(vv[0],u,2))%MOD*fact[cnt1-1]%MOD;
			return vis[u][p];
		}
		else{
			vis[u][p]=dfs(vv[0],u,p)*fact[cnt1-1]%MOD;
			return vis[u][p];
		}
	}
	if(vv.size()==2){
		if(p!=0){
			...
			vis[u][p]=(dfs(vv[0],u,1)*dfs(vv[1],u,2)%MOD+dfs(vv[0],u,2)*dfs(vv[1],u,1)%MOD)%MOD*fact[cnt1-2]%MOD;
			return vis[u][p];
		}
	}
	if(vv.size()>2){
