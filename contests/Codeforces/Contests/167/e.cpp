...
const int N = 1e6+10;//dont forget to change here

int n,k,fact[N],infact[N],cnt[N][20];
...
inline void SOLVE(int Case){
	...
	rep(i,2,n){
		rep(s,1,k){
			cnt[i][s]=cnt[i-1][s]*(s-1);
			if(s) cnt[i][s]+=cnt[i-1][s-1];
		}
	}
	int ans=0;
	rep(i,k,n){
		ans+=cnt[i][k]*C(n-1,i-1)%MOD;
		cout<<cnt[i][k]<<" "<<C(n-1,i-1)<<endl;
	}
	cout<<ans<<endl;
}
