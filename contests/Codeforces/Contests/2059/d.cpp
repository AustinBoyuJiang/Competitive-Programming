...
int n,s1,s2,m1,m2,dp[N][N],vis[N][N];
VI e1[N],e2[N];
set<PI> ss1,ss2;
...
inline void SOLVE(int Case){
	...
	while(!q.empty()){
		...
		if(vis[u1][u2]) break;
		vis[u1][u2]=1;
		for(int v1:e1[u1]) for(int v2:e2[u2]){
			if(dp[u1][u2]+abs(v1-v2)<dp[v1][v2]){
				dp[v1][v2]=dp[u1][u2]+abs(v1-v2);
				q.push({dp[v1][v2],{v1,v2}});
			}
		}
	}
	int ans=LLINF;
	for(PI p:ss1){
		if(ss2.find(p)!=ss2.end()){
			chkmin(ans,dp[p.fir][p.fir]);
			chkmin(ans,dp[p.sec][p.sec]);
		}
	}
	if(ans==LLINF){
		cout<<-1<<endl;
