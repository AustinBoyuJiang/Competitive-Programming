...
int n,cnt,root[2],ans[N],dep[N],dis[N][2],flag[N],far[N],id[N];
VI e[N];

void dfs(int u,int fa,int k){
	dep[u]=dep[fa]+1;
	if(dep[u]>dep[root[k]]) root[k]=u;
	for(auto v:e[u]){
		if(v==fa) continue;
		dfs(v,u,k);
	}
...
void SOLVE(int Case){
	...
	dep[0]=dis[0][0]=dis[0][1]=-1;
	dfs(1,0,0);
	dfs(root[0],0,1);
	dfs2(root[0],0,0);
	dfs2(root[1],0,1);
	rep(i,1,n){
		chkmax(dis[i][0],dis[i][1]);
		id[i]=dis[i][0];
	}
	sort(id+1,id+n+1);
	int j=1;
	rep(i,1,n){
		while(j<n&&id[j]-i<0) j++;
		cout<<min(n,j)<<" ";
	}
}
