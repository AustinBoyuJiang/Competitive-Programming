...
int n,tot,ans,a[N],cnt[MX],frac[N],infrac[N],dp[N][N];
set<int> num;
...
void SOLVE(int Case){
	...
	ans=frac[n];
	for(int x:num){
		if(is_prime(x)) prime.pb(x);
		else ans=ans*infrac[cnt[x]]%MOD;
	}
	dp[0][0]=ans;
	rep(i,1,prime.size()){
		rep(j,0,n){
			dp[i][j]+=dp[i-1][j]*infrac[cnt[prime[i-1]]]%MOD;
			if(j>0) dp[i][j]+=dp[i-1][j-1]*infrac[cnt[prime[i-1]]-1]%MOD;
			dp[i][j]%=MOD;
		}
	}
