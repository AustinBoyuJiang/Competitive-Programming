...
void SOLVE(int Case){
	...
	for(int x:num) tmp.pb(x);
	rep(i,0,n) pos[i]=lb(all(tmp),sum[i])-tmp.begin();
	Segtree<int> st(n,-INF,[](int x,int y){return max(x,y);});
	st.upd(pos[0],0);
	rep(i,1,n){
		dp[i]=max(dp[i-1],st.ask(0,pos[i])+i);
		st.upd(pos[i],dp[i]-i);
	}
	cout<<dp[n]<<endl;
}
