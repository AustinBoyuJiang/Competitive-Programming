/*
 * Author: Austin Jiang
 * Date: 3/23/2023 12:44:50 AM
 * Problem: Sequence Master
 * Source:
 * Description:
*/
...
void SOLVE(int Case){
	...
	if(n==1) chkmin(ans,abs(a[1]-a[2]));
	if(n==2) chkmin(ans,abs(a[1]-2)+abs(a[2]-2)+abs(a[3]-2)+abs(a[4]-2));
	if(n%2){
		cout<<ans<<endl;
		return;
	}
	int sum=0;
	rep(i,1,n*2) sum+=abs(a[i]+1);
	rep(i,1,n*2) chkmin(ans,sum-abs(a[i]+1)+abs(a[i]-n));
	cout<<ans<<endl;
}
