...
void SOLVE(int Case){
	cin>>A>>B>>C>>k;
	int LOA=A==1?1:qpow(10ll,(A-1));
	int HIA=qpow(10ll,A)-1;
	int LOB=B==1?1:qpow(10ll,(B-1));
	int HIB=qpow(10ll,B)-1;
	rep(a,LOA,HIA){
		int lo=qpow(10ll,C-1)-a;
		chkmax(lo,LOB);
		chkmax(lo,1ll);
		int hi=qpow(10ll,C)-1-a;
		chkmin(hi,HIB);
		if(k>hi-lo+1) k-=max(hi-lo+1,0ll);
		else{
			int b=lo+k-1;
			cout<<a<<" + "<<b<<" = "<<a+b<<endl;
			return;
