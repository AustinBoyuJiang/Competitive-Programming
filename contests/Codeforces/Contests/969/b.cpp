...
int n,w,a[N],b[N],par[N];
int fa[N],dep[N],siz[N],hson[N],f[N][20],mx[N][20];
PI lca[N];
VI e[N];
...
Tree<PI> rk;
void add(int u){
	rk.insert({dep[lca[u].fir],u*2});
	rk.insert({dep[lca[u].sec],u*2+1});
	for(int v:e[u]){
		add(v);
	}
}
...
inline void SOLVE(int Case){
	...
	init(1);
	rep(i,1,n){
		lca[i].fir=LCA(i,i%n+1);
		lca[i].sec=LCA(i,(i+n-2)%n+1);
	}
	rk.clear();
	dfs(1);
	VI ord;
