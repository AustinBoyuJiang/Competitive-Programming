...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		dp[i][0]=INF;
		dp[i][1]=-INF;
		chkmax(dp[i][1],max(dp[i-1][1]+a[i],abs(dp[i-1][1]+a[i])));
		chkmax(dp[i][1],max(dp[i-1][0]+a[i],abs(dp[i-1][0]+a[i])));
		chkmin(dp[i][0],min(dp[i-1][1]+a[i],abs(dp[i-1][1]+a[i])));
		chkmin(dp[i][0],min(dp[i-1][0]+a[i],abs(dp[i-1][0]+a[i])));
	}
	cout<<dp[n][1]<<endl;
}
