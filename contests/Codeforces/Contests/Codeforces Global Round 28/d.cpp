...
inline void SOLVE(int Case){
	...
	VI cnt(hi.size());
	rep(i,1,m){
		if(b[i]>a[1]) cnt[lb(all(hi),b[i])-hi.begin()]++;
	}
	VI sum(2*m);
	rep(i,1,(int)cnt.size()-2){
		cnt[i]+=cnt[i-1];
		sum[cnt[i]]++;
	}
	rep(i,1,2*m-1){
	...
	rep(k,1,m){
		ll ans=m/k;
		rep(i,0,m/k){
			ans+=(i+1)*1ll*sum[(i+1)*k-1];
			ans-=(i==0?0:i*1ll*sum[i*k-1])+sum[i*k+m%k];
		}
		cout<<ans<<" ";
	} cout<<endl;
}
