...
inline void SOLVE(int Case){
	...
	int cnt1=1;
	while(cnt1<n&&a[cnt1+1]) cnt1++;
	int cnt0=min(cnt1+1,n);
	while(cnt0<n&&!a[cnt0+1]) cnt0++;
	cnt0-=cnt1;
	if(cnt0<=cnt1){
		cout<<1<<" "<<n<<" "<<min(n,cnt1-cnt0+1)<<" "<<n-cnt0<<endl;
	}
	else{
		cout<<1<<" "<<n<<" "<<1<<" "<<max(1,n-cnt1)<<endl;
	}
}
