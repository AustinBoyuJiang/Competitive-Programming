...
int check(int i,int j){
	return abs(a[i].fir-a[i].sec)+abs(a[j].fir-a[j].sec);
}
...
inline void SOLVE(int Case){
	...
	sort(a+1,a+n+1,[](PI x,PI y){
		return -x.fir-x.sec-abs(x.fir-x.sec)>-y.fir-y.sec-abs(y.fir-y.sec);
	});
	rep(i,1,n){
		chkmax(mx,a[i].fir+a[i].sec-a[1].fir-a[1].sec-check(i,1));
	}
	sort(a+1,a+n+1,[](PI x,PI y){
		return x.fir+x.sec-abs(x.fir-x.sec)>y.fir+y.sec-abs(y.fir-y.sec);
	});
