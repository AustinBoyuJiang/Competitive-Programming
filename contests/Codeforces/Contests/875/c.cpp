...
void SOLVE(int Case){
	...
	ans=0;
	dfs(1,0);
	flag[ord[make_pair(0,1)]]=1;
	flag[ord[make_pair(1,0)]]=1;
	while(!q.empty()){
		int u=q.front().fir;
		int id=q.front().sec;
		chkmax(ans,id);
		...
		for(int v:e[u]){
			if(v==fa[u]) continue;
			int flagfa=flag[ord[make_pair(u,fa[u])]];
			if(ord[make_pair(u,fa[u])]<ord[make_pair(u,v)]){
				q.push({v,flagfa});
				flag[ord[make_pair(u,v)]]=flagfa;
			}
			else{
				q.push({v,flagfa+1});
				flag[ord[make_pair(u,v)]]=flagfa+1;
			}
		}
	}
	cout<<ans<<endl;
