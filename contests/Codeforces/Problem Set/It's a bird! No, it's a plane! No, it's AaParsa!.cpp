/*
 * Author: Austin Jiang
 * Date: 5/31/2023 10:41:35 PM
 * Problem: It's a bird! No, it's a plane! No, it's AaParsa!
 * Source:
 * Description:
*/
	...
	rep(st,0,n-1){
		...
		rep(t,1,n){
			...
			rep(i,0,n-1) rep(j,0,1){
				if(vis[i][j]) continue;
				if(u==-1||dist[i][j]<dist[u][d]) u=i,d=j;
			}
			vis[u][d]=true;
			for(auto edge:e[u]){
				int v=edge[0],w=edge[1],type=edge[2];
				if(type==1) chkmin(dist[(v+dist[u][d])%n][1],dist[u][d]+w);
				else if(d==1) chkmin(dist[v][d],dist[u][d]+w);
			}
		}
		rep(i,0,n-1){
			write(min(dist[i][0],dist[i][1]),' ');
		} putchar(endl);
	}
}
