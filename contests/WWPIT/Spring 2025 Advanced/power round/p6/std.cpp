...
int n,k,a[N],pos[N],les[N],cnt[N][N];
int fact[N],infact[N];
...
inline void SOLVE(int Case){
	...
	rep(unk,1,n){
		...
		per(i,k-1,0){
			int v=miss[i];
			cnt[unk][i]=cnt[unk][i+1];
			cnt[unk][i]+=v*C(i,unk-1)%MOD;
			cnt[unk][i]%=MOD;
		}
//		rep(i,mx==-1?0:les[mx],k-1){
//			cnt[mx]+=v*C(i,unk-1)%MOD;
//			cnx[mx]%=MOD;
//		}
	}
	int ans=0;
	rep(l,1,n){
		...
		rep(r,l,n){
			...
			int res=0;
			if(mx!=-1&&les[mx]>=unk){
				res+=mx*C(les[mx],unk)%MOD;
				res%=MOD;
//				cout<<mx*C(les[mx],unk)<<":"<<endl;
			}
			res+=cnt[unk][mx==-1?0:les[mx]];
			res%=MOD;
//			rep(i,mx==-1?0:les[mx],k-1){
//				int v=miss[i];
//				res+=v*C(i,unk-1)%MOD;
////				cout<<v*C(i,unk-1)<<","<<endl;
//			}
//			cout<<l<<" "<<r<<" "<<res<<" "<<fact[unk]*fact[k-unk]<<endl;
//			cout<<"---------------"<<endl;
			ans+=res*fact[unk]%MOD*fact[k-unk]%MOD;
			ans%=MOD;
		}
	}
	cout<<ans<<endl;
