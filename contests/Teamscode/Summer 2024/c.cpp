...
inline void SOLVE(int Case){
	...
	dp[1][0]=0;
	rep(i,2,mx){
		dp[i][1]=(dp[i-1][1]*inv(i-1)%MOD+(dp[i-1][0]+1)*(1-inv(i-1)+MOD)%MOD)%MOD;
		dp[i][0]=(dp[i-1][1]*inv(i-1)%MOD+dp[i-1][0]*(1-inv(i-1)+MOD)%MOD)%MOD;
	}
	rep(i,1,t){
		cout<<(dp[n[i]][1]*inv(n[i])%MOD+dp[n[i]][0]*(1-inv(n[i])+MOD)%MOD)%MOD<<endl;
	}
}
