...
int n,k,p[N],q[N],d[N],pow2[15];
bitset<M+10000> b[2],ad[15],full;
...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		cur^=1;
		b[cur]=full&(b[cur^1]<<p[i]);
//		b[cur]|=ad[d[i]]&(b[cur^1]>>pow2[d[i]]);
//		b[cur]|=(~ad[d[i]])&(b[cur^1]<<pow2[d[i]]);
		
		
		
		rep(j,0,M){
			if((j^pow2[d[i]])<=M&&b[cur^1][j^pow2[d[i]]]) b[cur].set(j);
		}
