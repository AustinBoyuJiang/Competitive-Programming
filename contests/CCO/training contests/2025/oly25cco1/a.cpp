...
unordered_map<int,PI> p;
Vec<pair<int,PI>> pp;

bool check(pair<int,PI> a){
	for(int i=0;i<pp.size();i+=(m<=1000?1:2)){
		auto b=pp[i];
		if(a.fir<b.fir&&b.sec.fir<=a.sec.fir&&a.sec.sec<=b.sec.sec){
			return true;
		}
		if(b.fir<a.fir&&a.sec.fir<=b.sec.fir&&b.sec.sec<=a.sec.sec){
			return true;
		}
	}
	return false;
