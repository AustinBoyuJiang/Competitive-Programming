...
int h,w,m,n,cnt[1<<N],val[1<<N];

struct constraint{
	int r1,c1,r2,c2,v;	
} p[N];

constraint merge(constraint a,constraint b){
	constraint res=a;
	chkmax(res.r1,b.r1);
	chkmin(res.r2,b.r2);
	chkmax(res.c1,b.c1);
...
int area(constraint a){
	return (a.r2-a.r1+1)*(a.c2-a.c1+1);
}
...
inline void SOLVE(int Case){
	cin>>h>>w>>m>>n;
	rep(i,0,n-1){
		cin>>p[i].r1>>p[i].c1>>p[i].r2>>p[i].c2>>p[i].v;
	}
	per(s,(1<<n)-1,0){
		constraint pp={1,1,h,w,m};
		rep(i,0,n-1){
			if((s>>i)&1){
				pp=merge(pp,p[i]);
			}
	...
	int ans=1;
	per(s,(1<<n)-1,1){
//		cout<<s<<" "<<cnt[s]<<" "<<val[s]<<endl;
		if(cnt[s]==0) continue;
		ans*=(qpow(val[s],cnt[s])-qpow(val[s]-1,cnt[s])+MOD)%MOD;
		ans%=MOD;
//		cout<<ans<<endl;
	}
	ans*=qpow(val[0],cnt[0]);
	ans%=MOD;
	cout<<ans<<endl;
}
