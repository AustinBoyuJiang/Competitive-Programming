...
inline void SOLVE(int Case){
	...
	for(char c:s){
		...
		while(q.size()>=3){
			int l=q.size()-1;
			bool ok=0;
			if(q[l-2].fir=='('&&q[l].fir==')'){
				q.pop_back();
				pair<char,VI> tmp=q.back();
				q.pop_back();
				q.pop_back();
				q.push_back(tmp);
				ok=1;
			}
			if(q[l].fir==' '&&q[l-1].fir=='<'&&q[l-2].fir==' '){
				pair<char,VI> a=q.back();
				q.pop_back();
				q.pop_back();
				pair<char,VI> b=q.back();
				q.pop_back();
				q.push_back({' ',Min(a.sec,b.sec)});
				ok=1;
			}
			if(q[l].fir==' '&&q[l-1].fir=='>'&&q[l-2].fir==' '){
				pair<char,VI> a=q.back();
