...
void solve(int n,int m,int k,int x,int y,int v){
//	cout<<n<<" "<<m<<" "<<k<<" "<<x<<" "<<y<<" "<<v<<endl;
	if(n==2&&m==2){
		a[x][y]=v;
		a[x+1][y]=v;
		a[x][y+1]=v;
	...
	if(res[n-2][m-2][k-1]-1){
		...
		solve(n-2,m-2,k-1,x+1,y+1,v+1);
		return;
	}
	for(int i=2;i<n;i+=2){
		for(int j=1;j<k;j++){
			if(res[i][m][j]-1&&res[n-i][m][k-j]-1){
				solve(i,m,j,x,y,v);
				solve(n-i,m,k-j,x+i,y,v+j);
				return;
			}
		}
	}
	for(int i=2;i<m;i+=2){
		for(int j=1;j<k;j++){
			if(res[n][i][j]-1&&res[n][m-i][k-j]-1){
				solve(n,i,j,x,y,v);
				solve(n,m-i,k-j,x,y+i,v+j);
				return;
			}
		}
	}
