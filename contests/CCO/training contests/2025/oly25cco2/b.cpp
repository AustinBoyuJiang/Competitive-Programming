...
int n,q,ans,father[N],w[N],f[N],dp[N];
multiset<int> s[N];
VI e[N];
...
void dfs(int u){
	...
	chkmax(dp[father[u]],dp[u]+w[u]);
	s[father[u]].insert(dp[u]+w[u]);
	f[u]=0;
	if(s[u].size()>=1){
		auto itr=s[u].rbegin();
		f[u]=*itr;
...
void upd(int u,int dpv,int wv){
	s[father[u]].erase(s[father[u]].find(dp[u]+w[u]));
	dp[u]=dpv;
	w[u]=wv;
	s[father[u]].insert(dp[u]+w[u]);
}
...
inline void SOLVE(int Case){
	...
	rep(i,1,q){
		...
		while(u){
			bool skip=true;
			if(dp[u]+w[u]>dp[father[u]]){
				upd(father[u],dp[u]+w[u],w[father[u]]);
				skip=false;
			}
			u=father[u];
			ans=(ans-f[u]%MOD+MOD)%MOD;
			f[u]=0;
			if(s[u].size()>=1){
				auto itr=s[u].rbegin();
				f[u]=*itr;
