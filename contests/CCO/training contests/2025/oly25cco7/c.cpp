...
int n,m,ans,frac[N],invfrac[N],a[N][N],vis[N][N];

int dfs(int n,int m,int x=0,int y=0,int flag=0){
	if(n==1&&m==1){
		if(!flag){
			ans++; 
		}
	...
	if(!vis[(x+1)%n][y]){
		vis[(x+1)%n][y]=1;
		int res=dfs(n,m,(x+1)%n,y,flag|a[(x+1)%n][y]);
		vis[(x+1)%n][y]=0;
		cnt+=res;
	}
	if(!vis[x][(y+1)%m]){
		vis[x][(y+1)%m]=1;
		int res=dfs(n,m,x,(y+1)%m,flag|a[x][(y+1)%m]);
		vis[x][(y+1)%m]=0;
		cnt+=res;
	}
	if(!flag){
