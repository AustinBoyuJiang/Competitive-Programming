...
int n,m,cnt,a[N],b[N],to[N],flag[N];
unordered_map<int,int> st,itr;
VI c,d,pp;
...
inline void SOLVE(int Case){
	...
	if(cyc.size()<2||m-cnt<2){
		cout<<cyc.size()<<endl;
		rep(i,1,cyc.size()){
			cout<<cyc[i-1].size()<<endl;
			rep(j,0,(int)cyc[i-1].size()-1){
				cout<<cyc[i-1][j]<<" \n"[j==(int)cyc[i-1].size()-1];
			}
		}
		return;
	}
	int need=max(0,(int)cyc.size()-(m-cnt));
	cout<<need+2<<endl;
	rep(i,m-cnt+1,cyc.size()){
		cout<<cyc[i-1].size()<<endl;
		rep(j,0,(int)cyc[i-1].size()-1){
			cout<<cyc[i-1][j]<<" \n"[j==(int)cyc[i-1].size()-1];
		}
	...
	VI v;
	rep(i,1,min(m-cnt,(int)cyc.size())){
		for(int x:cyc[i-1]){
			v.pb(x);
		}
	}
	...
	v.clear();
	cout<<min(m-cnt,(int)cyc.size())<<endl;
	per(i,min(m-cnt,(int)cyc.size()),1){
		cout<<cyc[i-1][0]<<" \n"[i==1];
	}
}
