...
//	cout<<(*itr).fir<<endl;
	...
	rep(i,k,n){
		insert({-a[i].fir,i});
		Remove({-a[i-k].fir,i-k});
		int res=-sum-(a[i].sec-a[i-k+1].sec)*2;
//		cout<<i-k+1<<" "<<i<<" "<<res<<endl;
		chkmax(ans,res);
	}
	return ans;
}
...
inline void SOLVE(int Case){
	...
	rep(i,max(l-50,m),min(l+50,n)){
		chkmax(ans,check(i));
	}
	rep(i,m,min(m+50ll,n)){
		chkmax(ans,check(i));
		chkmax(ans,check(n-i+m));
	}
	cout<<ans<<endl;
}
