...
int another(int pos){
	return p[a[pos]][0]+p[a[pos]][1]-pos;
}

bool solve(int Case){
	string res=Case==0?"L":"R";
	int l=1+(Case==0),r=n*2-(Case==1),u=Case==0?1:n*2;
	int ll=another(u),rr=ll;
	b[1]=a[u];
	rep(i,2,n){
		int pl=another(ll-1);
		int pr=another(rr+1);
