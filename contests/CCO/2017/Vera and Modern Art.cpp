/*
 * Author: Austin Jiang
 * Date: 5/19/2024 1:53:38 PM
 * Problem: Vera and Modern Art
 * Source:
 * Description:
*/
...
unordered_map<pair<ll,ll>,int,pair_hash> s[D][D];
...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		...
		cin>>p[i].y;
		cin>>p[i].c;
		int dx=(int)log2(p[i].x);
		int dy=(int)log2(p[i].y);
		p[i].x-=pow2[dx];
		p[i].y-=pow2[dy];
		s[dx][dy][{p[i].x,p[i].y}]+=p[i].c;
	}
	while(q--){
		ll r,c;
		int ans=0;
		...
		rep(i,0,60) rep(j,0,60){
			if(r<pow2[i]||c<pow2[j]) continue;
			auto itr=s[i][j].find({r%pow2[i],c%pow2[j]});
			if(itr!=s[i][j].end()) ans+=(*itr).sec;
		}
		cout<<ans<<endl; 
	}
}
