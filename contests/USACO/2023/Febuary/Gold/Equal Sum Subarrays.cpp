...
void SOLVE(int Case){
	...
	rep(i,1,n){
		VI val;
		rep(l,1,i-1) rep(r,l,i-1){
			val.pb(sum[r]-sum[l-1]);
		}
		rep(l,i+1,n) rep(r,l,n){
			val.pb(sum[r]-sum[l-1]);
		}
		sort(all(val));
		int ans=INF;
		rep(l,1,i) rep(r,i,n){
			int x=sum[r]-sum[l-1];
			auto a=ub(all(val),x)-1;
			auto b=lb(all(val),x);
			if(a>=val.begin())chkmin(ans,abs(val[a-val.begin()]-x));
			if(b!=val.end()) chkmin(ans,abs(val[b-val.begin()]-x));
		}
		cout<<ans<<endl;
	}
}
