...
int n,m,tot,f[N],pow10[N];
PI dp[N];

PI operator + (const PI a,const PI b){
	return {a.fir+b.fir,(a.sec*1ll*pow10[b.fir]%MOD+b.sec)%MOD};
} 

int find(int x){
	if(f[x]==x) return x;
	int ffx=find(f[x]);
	if(f[x]==ffx) return ffx;
	dp[x]=dp[x]+dp[f[x]];
	return f[x]=ffx;
}

PI get(int x){
	if(find(x)==x) return dp[x];
	return dp[x]+get(f[x]);
}
...
inline void SOLVE(int Case){
	...
	rep(i,1,m){
		...
		PI dx=get(x);
		PI dy=get(y);
		dp[fx]=dp[fx]+make_pair(1,i)+dy;
		dp[fy]=dp[fy]+make_pair(1,i)+dx;
		f[fx]=f[fy]=++tot;
	}
	rep(i,1,n){
		cout<<get(i).sec<<endl;
