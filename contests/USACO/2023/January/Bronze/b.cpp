...
int n,m,ans=INF,a[N],c[N],flag[N];
...
bool ok(){
	rep(i,1,100) if(c[i]<a[i]) return 0;
	return 1;
}
...
void dfs(int pos){
	if(pos==m){
		...
		rep(i,1,m){
			if(flag[i]){
				cnt+=b[i].c;
				rep(j,b[i].a,b[i].b) c[j]+=b[i].v;
			}
		}
		if(ok()) chkmin(ans,cnt);
		return;
	}
	flag[pos+1]=0;
	dfs(pos+1);
