...
const int N = 110;
int n,m,ans,a[N],vis[N],fst[N],lst[N],adj[N][N],flag[N];
...
void solve(int Case){
	...
	rep(i,1,m){
		...
		rep(j,1,k){
			cin>>a[j];
			if(!ans) continue;
			if(vis[a[j]]&&i-vis[a[j]]>1) ans=0;
			vis[a[j]]=i;
			if(!stk.empty()&&stk.top()==a[j]) stk.pop();
			else stk.push(a[j]);
			if(!fst[a[j]]) fst[a[j]]=i;
		}
		if(!stk.empty()){
			ans=0;
		}
		...
		rep(x,1,k-1){
			rep(y,x+1,k){
				if(adj[a[x]][a[y]]==-1||adj[a[y]][a[x]]==1){
					ans=0;
					break;
				}
				adj[a[x]][a[y]]=1;
