...
const int N = 210;
int n,K,ans=INF,a[N],cnt[N],sum[N],res[N][N],dp[N][N][10];
...
void solve(int Case){
	...
	rep(l,1,n*2) rep(r,l,n*2){
		res[l][r]=sum[l+1]-sum[r+1]-cnt[r+1]*(r-l);
	}
	if(K==1){
		int ans=INF;
		rep(i,1,n){
			chkmin(ans,res[i][i+n-1]);
		}
		cout<<ans<<endl;
		return;
	}
	...
	rep(st,1,n){
		rep(i,st+1,st+n-1){
			dp[st][i][1]=0;
			dp[st][i][2]=res[st][i-1];
			rep(k,3,K) rep(j,st+1,i-1){
				chkmin(dp[st][i][k],dp[st][j][k-1]+res[j][i-1]);
			}
			chkmin(ans,dp[st][i][K]+res[i][st+n-1]);
//			cout<<st<<" "<<i<<": "<<dp[st][i][K]<<" "<<res[i+1][st+n-1]<<endl;
		}
	}
	cout<<ans<<endl;
}
