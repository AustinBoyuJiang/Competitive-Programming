...
const int N = 1e6+10;
int n,m,cnt,ans,sum,A[N],B[N],mx[N];
...
void solve(int Case){
	...
	rep(i,1,n){
		int a,b;
		cin>>a>>b;
		if(a>=m) chkmax(ans,1000*b/a);
		if(a<=1100){//由于b一定小于1000，当a大于1000时肯定不会和别人组合 
			sum+=a;
			A[++cnt]=a;
			B[cnt]=b;
		}
	...
	rep(i,1,cnt){
		per(j,sum,0){
			if(j-A[i]>=0){
				chkmax(mx[j],mx[j-A[i]]+B[i]);//分母固定，分子越大答案越大 
			}
		}
	}
	rep(i,m,sum){
		chkmax(ans,1000*mx[i]/i);
	}
	cout<<ans<<endl;
}
