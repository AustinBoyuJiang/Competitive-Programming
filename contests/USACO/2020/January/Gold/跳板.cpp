...
int ask(int rt,int l,int r,int x,int y){
	...
	int mid=l+r>>1;
	if(y<=mid) return ask(rt<<1,l,mid,x,y);
	else if(x>mid) return ask(rt<<1|1,mid+1,r,x,y);
	return max(ask(rt<<1,l,mid,x,mid),ask(rt<<1|1,mid+1,r,mid+1,y));
}
...
void solve(int Case){
	...
	rep(i,1,m){
		...
		p[i].id=p[i+m].id=i;
		p[i].dis=p[i+m].y+p[i+m].x-p[i].x-p[i].y;
		p[i+m].tp=1;
	...
	rep(i,1,m*2){
		p[i].x=lb(all(num),p[i].x)-num.begin()+1;
		p[i].y=lb(all(num),p[i].y)-num.begin()+1;
	}
	sort(p+1,p+m*2+1,cmp);
	rep(i,1,m*2){
		if(p[i].tp==0){
			dp[p[i].id]=ask(1,1,m*4,1,p[i].x)+p[i].dis;
			chkmax(ans,dp[p[i].id]);
		}
		else{
			int x=max(ask(1,1,m*4,p[i].x,p[i].x),dp[p[i].id]);
			update(1,1,m*4,p[i].x,x);
		}
	}
	cout<<n*2-ans<<endl;
}
