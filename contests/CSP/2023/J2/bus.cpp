...
inline void SOLVE(int Case){
	...
	while(!q.empty()){
		...
		for(PI edge:e[u]){
			int v=edge.fir;
			int w=edge.sec;
			int res1=dist[u][d].fir+1;
			int res2=max(dist[u][d].sec,w-dist[u][d].fir);
			if(res1+res2<dist[v][(d+1)%k].fir+dist[v][(d+1)%k].sec){
				dist[v][(d+1)%k]={res1,res2};
				q.push({res1+res2,{v,(d+1)%k}});
			}
		}
	}
	int ans=dist[n][0].fir+dist[n][0].sec;
	if(ans>=INF){
		cout<<-1<<endl;
		return;
	}
