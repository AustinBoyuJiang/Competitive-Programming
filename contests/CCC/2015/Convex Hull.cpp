...
int n,m,k,s,t,ans=INF,dist[N][K],vis[N][K];
VEC<pair<int,PI>> e[N];
PQG<pair<int,PI>> q;
...
void SOLVE(int Case){
	...
	while(!q.empty()){
		...
		for(auto edge:e[u]){
			...
			int h=edge.sec.sec;
			if(d+h>=k) continue;
			if(dist[u][d]+w<dist[v][d+h]){
				dist[v][d+h]=dist[u][d]+w;
				q.push({dist[v][d+h],{v,d+h}});
			}
		}
	}
	rep(i,0,k-1){
