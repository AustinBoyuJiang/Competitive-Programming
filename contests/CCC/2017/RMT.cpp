...
const int BN = sqrt(N)+10;

int n,m,q,tot,siz,val[N],sum[BN],col[N],rnk[N],cnt[M],pre[BN][M],block[N];
VI pos[M];

int calc(int x){
	int len=pos[col[x]].size();
	int id=rnk[x]-cnt[col[x]]%len;
	return val[pos[col[x]][(id+len)%len]];
}
...
void SOLVE(int Case){
	...
	rep(i,1,tot){
		int end=min(n,i*siz);
		per(j,n,1){
			pre[i][col[(end-j+n)%n+1]]=(end-j+n)%n+1;
		}
	}
	rep(i,1,q){
		int opt=read();
		if(opt==1){
			...
			if(block[l]==block[r]){
				rep(i,l,r) ans+=calc(i);
			}
			else{
				int a=block[l]+1;
				int b=block[r]-1;
				rep(i,a,b) ans+=sum[i];
				rep(i,l,block[l]*siz) ans+=calc(i);
				rep(i,(block[r]-1)*siz+1,r) ans+=calc(i);
			}
			write(ans,endl);
		}
		else{
			...
			rep(i,1,tot){
				int len=pos[x].size();
				sum[i]+=val[pos[x][(rnk[pre[(i-2+tot)%tot+1][x]]-cnt[x]%len+len)%len]];
				sum[i]-=val[pos[x][(rnk[pre[i][x]]-cnt[x]%len+len)%len]];
			}
			cnt[x]++;
		}
	}
