/*
 * Author: Austin Jiang
 * Date: 1/25/2023 12:49:21 PM
 * Problem: Combining Riceballs
 * Source:
 * Description:
*/
...
void SOLVE(int Case){
	...
	rep(len,2,n){
		rep(l,1,n-len+1){
			int r=l+len-1;
			rep(mid,l,r-1){
				if(dp[l][mid]==dp[mid+1][r]&&dp[l][mid]>0){
			...
			int j=r;
			rep(i,l,r-1){
				while(j>l&&dp[j][r]<dp[l][i]) j--;
				if(j<=i) break;
				if(dp[l][i]==dp[j][r]&&dp[l][i]>0&&dp[i+1][j-1]>0){
					dp[l][r]=dp[l][i]+dp[i+1][j-1]+dp[j][r];
					break;
				}
			}
			chkmax(ans,dp[l][r]);
