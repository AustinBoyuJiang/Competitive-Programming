...
void init(){
	...
	int Base=1;
	rep(i,1,len){
		for(int x:pre[i]) for(int y:suf[i]){
			int mid=max(x%10,y/Base);
			rep(k,mid+1,9){
				v.pb(x*Base*100+k*Base*10+y);
			}
		}
		Base*=10;
	}
