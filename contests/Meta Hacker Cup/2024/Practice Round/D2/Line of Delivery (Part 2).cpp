/*
 * Author: Austin Jiang
 * Date: 9/22/2024 2:44:23 AM
 * Problem: Line of Delivery (Part 2)
 * Source:
 * Description:
*/
...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		int lst=pos.size();
		per(j,(int)pos.size()-1,0){
			if(p[i]+(int)pos.size()-j-1>=pos[j]-offset){
				lst--;
			}
			else{
				break;
		...
		pos.insert(pos.begin()+lst,p[i]+offset+pos.size()-lst);
		rep(j,0,lst){
			pos[j]++;
		}
		offset++;
...
//	-----------------------
	int ans=1;
	rep(i,2,n){
		if(abs(pos[i-1]-g)<abs(pos[ans-1]-g)){
			ans=i;
		}
	}
	cout<<"Case #"<<Case<<": ";
	cout<<ans<<" "<<abs(pos[ans-1]-g)<<endl;
}
