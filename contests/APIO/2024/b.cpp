...
ll solve(int n,int m,int k,VI p,VI u,VI v,VI s,VI t,VI c,VI l,VI r){
	rep(i,0,n-1){
		ord[i].clear();
	}
	int tot=0;
	...
	if(ord[0].empty()) return -1;
	rep(i,0,n-1){
		sort(all(ord[i]));
		rep(j,0,(int)ord[i].size()-2){
			e[ord[i][j].sec].pb({ord[i][j+1].sec,0}); 
		}
	}
	fill(dist+1,dist+tot+1,LLINF);
	fill(vis+1,vis+tot+1,0);
	dist[ord[0][0].sec]=0;
	PQG<pair<ll,int>> q;
	q.push({0,ord[0][0].sec});
	while(!q.empty()){
		int u=q.top().sec;
		q.pop();
		if(vis[u]) continue;
	...
	ll ans=dist[ord[n-1][(int)ord[n-1].size()-1].sec];
