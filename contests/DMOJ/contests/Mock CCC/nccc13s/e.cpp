...
void push_up(int rt,int l,int r){
	if(flag[rt]) st[rt]={r-l+1,1,l,r};
	else if(r==l) st[rt]={0,0,0,0};
	else{
		st[rt]={st[lc].mx+st[rc].mx,st[lc].cnt+st[rc].cnt,st[lc].l,st[rc].r};
		if(st[lc].cnt&&st[rc].cnt&&st[lc].r+1==st[rc].l) st[rt].cnt--;
		if(!st[lc].cnt) st[rt].l=st[rc].l;
		if(!st[rc].cnt) st[rt].r=st[lc].r;
	}
}
...
void update(int rt,int l,int r,int x,int y,int k){
	...
	else update(lc,l,mid,x,mid,k),update(rc,mid+1,r,mid+1,y,k);
	push_up(rt,l,r);
}
...
inline void SOLVE(int Case){
	...
	int ans=0,ans2=0;
	rep(i,0,10000){
		for(pair<PI,int> p:row[i]){
			update(1,0,10000,p.fir.fir,p.fir.sec,p.sec);
		}
		ans+=st[1].mx;
		ans2+=st[1].cnt*2;
	}
