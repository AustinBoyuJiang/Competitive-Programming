...
int get(int sx,int sy,int tx,int ty){
	return C(sx+sy-tx-ty,sx-tx);
}

inline void SOLVE(int Case){
	cin>>n;
	fact[0]=1;
	rep(i,1,n*2) fact[i]=fact[i-1]*i%MOD;
	rep(i,1,n) cin>>a[i];
	rep(i,1,n) cin>>b[i];
	int cnt1=0,cnt2=0;
	while(cnt1<n&&a[cnt1+1]==b[1]) cnt1++;
	while(cnt2<n&&b[cnt2+1]==a[1]) cnt2++;
	int ans=(get(n,n,cnt1,0)+get(n,n,0,cnt2)-get(n,n,cnt1,cnt2)+MOD)%MOD;
	cout<<ans<<endl;
}
