...
inline void SOLVE(int Case){
	...
	int ans=0;
	rep(mid,0,m){
		rep(dis,1,min(mid,m-mid)){
			vis.clear();
			vis[0]=0;
			rep(y,1,n){
				ll a=get(0,y,mid-dis,mid);
				ll b=get(0,y,mid,mid+dis);
				if(vis.find(a-b)!=vis.end()) chkmax(ans,dis*2*(y-vis.find(a-b)->sec));
				if(vis.find(a-b)!=vis.end()) vis[a-b]=min(vis.find(a-b)->sec,y);
				else vis[a-b]=y;
			}
		}
	}
