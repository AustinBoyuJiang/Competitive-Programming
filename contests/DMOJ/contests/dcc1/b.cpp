...
int get(int n){
	return n%MOD*(n+1)%MOD*(n*2+1)%MOD*inv(6ll)%MOD;
}
...
inline void SOLVE(int Case){
	...
	while(t--){
		int a,b,ans=0;
		cin>>a>>b;
		int l=0,r=1e10,pos=-1;
		while(l<=r){
			int c=l+r>>1;
			if(c*c<a+b-c*c&&c*c<min(a,b)){
				pos=c;
				l=c+1;
			}
			else{
		...
		while(l<=r){
			int c=l+r>>1;
			if(a+b-c*c<c*c&&a+b-c*c<min(a,b)){
				pos2=c;
				r=c-1;
			}
			else{
		...
		ans-=get(pos3)%MOD-get(pos2-1)%MOD-MOD;
		ans%=MOD;
		ans+=(pos3-(pos2-1+MOD)%MOD+MOD)%MOD*((a+b+1)%MOD)%MOD;
		ans%=MOD;
		ans+=(pos2%MOD-1-pos%MOD+MOD)%MOD*(min(a,b)%MOD+1)%MOD;
		ans%=MOD;
		cout<<(ans+MOD)%MOD<<endl;
//		cout<<pos<<" "<<pos2<<" "<<pos3<<endl;
	}
}
