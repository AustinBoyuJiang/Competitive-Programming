...
int n,m,lst,a[N][N],f[N*N],ans[N*N];
Vec<pair<PI,PI>> e[N];
VI stk[N*N];
...
void merge(int x,int y){
	...
	if(stk[fx].size()>stk[fy].size()) swap(stk[fx],stk[fy]);
	for(int u:stk[fx]){
		stk[fy].pb(u);
	}
	stk[fx]=VI();
	...
	rep(i,1,n){
		rep(j,1,m){
			if(a[i][j]==0){
				...
				while(a[i][j+d+1]) d++;
				if(j+d+1<=m){
					e[d?d:max(n,m)].pb({{i,j},{i,j+d+1}});
				}
				d=0;
				while(a[i+d+1][j]) d++;
				if(i+d+1<=n){
					e[d?d:max(n,m)].pb({{i,j},{i+d+1,j}});
				}
			}
		}
	}
