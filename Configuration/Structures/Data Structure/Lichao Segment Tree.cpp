...
struct LichaoSegtree_min{
	...
	inline int calc(int id,int x){
		if(id==-1) return INF;
		return seg[id].sec*x+seg[id].fir;
	}
	
	inline int check(int a,int b,int x){
		if(calc(a,x)<calc(b,x)) return a;
		return b;
	}
	
	inline int ask(int x,int rt,int l,int r){
	...
	inline void upd(int id,int rt,int l,int r){
		int mid=l+r>>1;
		int a=check(st[rt],id,mid),b=st[rt]+id-a; st[rt]=a;
		if(check(a,b,l)==b) upd(b,lc,l,mid);
		if(check(a,b,r)==b) upd(b,rc,mid+1,r);
	}
...
struct LichaoSegtree_max{
	...
	inline int ask(int x,int rt,int l,int r){
		if(l==r) return calc(st[rt],x);
		int mid=l+r>>1;
		if(x<=mid) return max(ask(x,lc,l,mid),calc(st[rt],x));
		else return max(ask(x,rc,mid+1,r),calc(st[rt],x));
	}
