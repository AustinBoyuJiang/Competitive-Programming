...
int n,m,R,P,a[N],v[N][M],dp[N][N][N][M][M];
...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		cin>>a[i];
		rep(j,1,n) rep(k,1,n) rep(l,1,m) rep(w,1,m){
			dp[i][j][k][l][m]=-INF;
		}
	}
	rep(i,1,m){
	...
	rep(i,1,n){
		dp[i][i][1][a[i]][1]=v[a[i]][1];
	}
	rep(len,2,n){
		rep(l,1,n-len+1){
			int r=l+len-1;
			rep(rlen,1,len){
				rep(type,1,m) rep(lvl,1,R){
					dp[l][r][rlen][type][lvl]=dp[l][r-k][rlen][type][lvl]+dp[r-k+1][r][any][type][lvl]
				}
			} 
		}
	}
