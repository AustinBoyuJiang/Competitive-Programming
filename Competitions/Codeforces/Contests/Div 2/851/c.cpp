...
template<class T,class Fun=function<T(const T,const T)>> struct Segtree{ // for maximum and minimum
	...
	T ask(int x,int y){return query(1,1,n,x,y);}
	void update(int rt,int l,int r,int x,int v){
		...
		st[rt]=f(st[lc],st[rc]);}
	void upd(int x,int y){update(1,1,n,x,y);}
};
...
void SOLVE(int Case){
	...
	int c1=n-c2;
	int x=0;
	if(n%4==1) swap(c1,c2),x=1;
	rep(i,0,n/2){
		if(i%2==1){
			cout<<(c2+(n-c2+x)/2)-i/2<<" "<<n*2-((c2+(n-c2+x)/2)+(i+1)/2)+1<<endl;
			cout<<(c2+(n-c2+x)/2)+(i+1)/2<<" "<<n*2-((c2+(n-c2+x)/2)-i/2)+1<<endl;
		}
		else{
			cout<<(c1+1)/2+i/2<<" "<<n*2-((c1+1)/2-i/2)+1<<endl;
			if(i!=0) cout<<(c1+1)/2-i/2<<" "<<n*2-((c1+1)/2+i/2)+1<<endl;
		}
	}
}
