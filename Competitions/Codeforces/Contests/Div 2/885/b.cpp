...
void SOLVE(int Case){
	...
	rep(i,1,n){
		if(i-pre[a[i]]>dis[a[i]][0]){
			dis[a[i]][1]=dis[a[i]][0];
			dis[a[i]][0]=i-pre[a[i]];
		}
		else{
			dis[a[i]][1]=max(dis[a[i]][1],i-pre[a[i]]);
		}
		pre[a[i]]=i;
	}
	rep(i,1,k){
		if(n+1-pre[i]>dis[i][0]){
			dis[i][1]=dis[i][0];
			dis[i][0]=n+1-pre[i];
		}
		else{
			dis[i][1]=max(dis[i][1],n+1-pre[i]);
		}
	}
	int ans=INF;
	rep(i,1,k){
//		cout<<dis[i][0]<<" "<<dis[i][1]<<endl;
		ans=min(ans,max(dis[i][1],(dis[i][0]+1)/2));
	}
	cout<<ans-1<<endl;
}
