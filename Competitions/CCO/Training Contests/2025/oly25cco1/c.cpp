...
inline void SOLVE(int Case){
	...
	rep(i,1,q){
		...
		for(PI edge:edges){
			if(edge.fir==s||edge.sec==s) continue;
			e[edge.fir].pb(edge.sec);
			e[edge.sec].pb(edge.fir);
		}
		dfs(t);
		VI ord;
		rep(j,1,n){
		...
		bool ok=1;
		rep(j,0,min((int)ord.size()-1,200)){
			if(check(ord[j])){
				ok=0;
				break;
			}
