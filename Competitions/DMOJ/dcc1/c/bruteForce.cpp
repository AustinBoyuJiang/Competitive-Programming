...
bool check(int x1,int len,int y1,int y2){
	int sum1=0,sum2=0;
	rep(i,y1,y2){
		rep(j,x1,x1+len/2-1){
			sum1+=a[i][j];
		}
		rep(j,x1+len/2,x1+len-1){
			sum2+=a[i][j];
		}
	}
	return sum1==sum2;
...
inline void SOLVE(int Case){
	...
	int ans=0;
	rep(i,1,m){
		for(int len=2;i+len-1<=m;len+=2){
			rep(y1,1,n) rep(y2,y1,n){
				if(check(i,len,y1,y2)){
					chkmax(ans,len*(y2-y1+1));
				}
			}
		}
	}
