...
int n,m,q,src[N],to[N],dis[N],f[N][20],dep[N],up[N],dw[N],sum1[N],sum2[N];
VI e[N];
...
inline void SOLVE(int Case){
	...
	rep(i,1,q){
		...
		y^=lst;
		int lca=LCA(x,y);
		int ans=sum1[x]-sum1[lca]+sum2[y]-sum2[lca]+dis[y]*2;
		cout<<ans<<endl;
		lst=ans;
	}
}
