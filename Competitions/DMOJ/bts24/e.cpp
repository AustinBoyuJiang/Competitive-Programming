...
inline void SOLVE(int Case){
	...
	per(i,n,1){
		...
		rep(k,1,X-1){
			if(k<=pos[a[i]].size()){
				chkmin(val[k],pos[a[i]][pos[a[i]].size()-k]);
			}
		}
		for(int id:ll[i]){
			int k=q[id].k;
			if(k<X) chkmin(q[id].ans,val[k]);
			else for(int x:hv){
				if(k<=pos[x].size()){
					chkmin(q[id].ans,pos[x][pos[x].size()-k]);
				}
			}
		}
	}
	rep(i,1,m){
		if(q[i].ans>q[i].r) cout<<-1<<endl;
		else cout<<a[q[i].ans]<<endl;
	}
}
