...
const int N = 510;
int n,m,ans,tot,a[N][N],d[N][N],flag[N][N][4],vis[N][N],res[N][N];
...
inline int dfs(int x,int y,int col){
	...
	res[x][y]=0;
	if(ok(nx,ny)){
		res[x][y]=dfs(nx,ny,col);
		vis[x][y]=col=vis[nx][ny];
	}
	rep(i,0,3){
		int nx=x+dir[i][0];
		int ny=y+dir[i][1];
		if(a[nx][ny]){
			bool has=0;
			rep(j,0,3) if(flag[nx][ny][j]==col) has=1;
			if(!has) res[x][y]++;
			flag[nx][ny][i]=col;
		}
	}
