...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		...
		dp[i]=max(dp[i-1],query(root[pos],0,(int)rk.size()-1,1,lb(all(rk),p[i].fir)-rk.begin())+1);
		update(root[i],root[i-1],0,(int)rk.size()-1,lb(all(rk),p[i].fir+p[i].sec)-rk.begin(),dp[i]);
	}
	cout<<dp[n]<<endl;
}
