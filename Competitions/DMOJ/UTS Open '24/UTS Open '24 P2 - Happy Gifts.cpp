...
inline void SOLVE(int Case){
	...
	sort(all(c[0]),[](int a,int b){
		return a>b;
	});
	sort(all(c[1]),[](int a,int b){
		return a>b;
	});
	if(c[0].size()%2) c[0].pb(0);
	int i=0,j=0;
	while(k>=2&&i<c[0].size()&&j<c[1].size()){
		if(c[0][i]+c[0][i+1]>c[1][j]){
			ans+=c[0][i]+c[0][i+1];
			i+=2;
			k-=2;
		}
