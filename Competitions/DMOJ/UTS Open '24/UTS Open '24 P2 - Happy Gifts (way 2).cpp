...
int n,k,m1,m2,a[N],b1[N],b2[N],sum1[N],sum2[N];
...
inline void SOLVE(int Case){
	...
	sort(b1+1,b1+m1+1,[](int a,int b){
		return a>b;
	});
	sort(b2+1,b2+m2+1,[](int a,int b){
		return a>b;
	});
	rep(i,1,m1){
		sum1[i]=sum1[i-1]+b1[i];
	}
	rep(i,1,m2){
		sum2[i]=sum2[i-1]+b2[i];
	}
	int ans=0;
	rep(i,0,m2){
		if(i*2>k) break;
		chkmax(ans,sum2[i]+sum1[min(k-i*2,m1)]);
	}
	cout<<ans<<endl;
}
