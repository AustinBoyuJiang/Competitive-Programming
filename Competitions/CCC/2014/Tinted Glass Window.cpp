/*
 * Author: Austin Jiang
 * Date: 1/30/2023 1:45:25 AM
 * Problem: Tinted Glass Window
 * Source:
 * Description:
*/
...
void SOLVE(int Case){
	...
	rep(i,1,n){
		p[i].x1=lb(all(xv),p[i].x1)-xv.begin()+2;
		p[i].x2=lb(all(xv),p[i].x2)-xv.begin()+2;
		p[i].y1=lb(all(yv),p[i].y1)-yv.begin()+2;
		p[i].y2=lb(all(yv),p[i].y2)-yv.begin()+2;
		a[p[i].x1][p[i].y1]+=p[i].val;
		a[p[i].x1][p[i].y2]-=p[i].val;
		a[p[i].x2][p[i].y1]-=p[i].val;
		a[p[i].x2][p[i].y2]+=p[i].val;
	}
	int ans=0;
	rep(i,1,n*2){
		rep(j,1,n*2){
			a[i][j]=a[i][j]+a[i][j-1]+a[i-1][j]-a[i-1][j-1];
			if(a[i][j]>=m) ans+=(x[i+1]-x[i])*(y[j+1]-y[j]);
		}
	}
	cout<<ans<<endl;
}
