...
struct apa{ //arbitrary-precision arithmetic
	...
	inline void load(string x,int arr[]){
		for(int i=0;i<x.size();i++)
			arr[i]=x[x.size()-i-1]-'0';
	}
	
	inline void store(int len,string &x,int arr[]){
		x="";
	...
	inline bool operator> (const apa &other){
	...
	inline apa operator+ (const apa &other){
		string a=this->num,b=other.num;
...
inline apa C(int n,int m){
	return P(n,m)/P(m,m);
}

inline apa solve(int n,int m){
	return C(n+m,n);
}
...
signed main(){
	...
	cin.tie(0);
	cin>>n>>m;
	cout<<(solve(n,m)-solve(m-1,n+1)).num<<endl;
	return 0;
}
