...
void SOLVE(int Case){
	...
	st[1].id=1;
	st[m-1].id=m-1;
	sort(st+1,st+m+1,cmpx);
	rep(i,1,m-1){
		if(st[i].x==st[i+1].x){
			e[st[i].id].pb({st[i+1].id,(st[i+1].y-st[i].y)*2});
			e[st[i+1].id].pb({st[i].id,(st[i+1].y-st[i].y)*2});
		}
	}
	sort(st+1,st+m+1,cmpy);
	rep(i,1,m-1){
		if(st[i].y==st[i+1].y){
			e[st[i].id+m].pb({st[i+1].id+m,(st[i+1].x-st[i].x)*2});
			e[st[i+1].id+m].pb({st[i].id+m,(st[i+1].x-st[i].x)*2});
		}
	}
	rep(i,1,m){
		e[i].pb({i+m,i!=1&&i!=m});
