/*
 * Author: Austin Jiang
 * Date: 1/15/2023 9:05:34 PM
 * Problem: Revamping Trails
 * Source:
 * Description:
*/
...
const int N = 5e4*21+10;
int n,m,k,ans=INF,dist[N],vis[N];
VPI e[N];
...
void SOLVE(int Case){
	...
	rep(i,1,m){
		...
		cin>>u>>v>>w;
		rep(j,0,k-1){
			e[id(u,j)].pb({id(v,j),w});
			e[id(v,j)].pb({id(u,j),w});
			e[id(u,j)].pb({id(v,j+1),w/2});
			e[id(v,j)].pb({id(u,j+1),w/2});
		}
		e[id(u,k)].pb({id(v,k),w});
		e[id(v,k)].pb({id(u,k),w});
	}
	...
	rep(i,0,k) chkmin(ans,dist[id(n,i)]);
	cout<<ans<<endl;
}
