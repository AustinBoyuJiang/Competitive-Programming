...
inline void SOLVE(int Case){
	...
	array<int,4> ans={INF,0,0,0},res;
	rep(a,1,n-1) rep(b,a+1,n){
		int i=0,j=0;
		if(i>=e[a].size()) continue;
		if(j>=e[b].size()) continue;
		if(e[a][i]==b) i++;
		if(e[b][j]==a) j++;
		if(i>=e[a].size()) continue;
		if(j>=e[b].size()) continue;
		...
		i++;
		j++;
		if(i>=e[a].size()&&j>=e[b].size()) continue;
		if(i<e[a].size()&&e[a][i]==b) i++;
		if(j<e[b].size()&&e[b][j]==a) j++;
		if(i>=e[a].size()&&j>=e[b].size()) continue;
		else if(j>=e[b].size()) d=e[a][i];
		else if(i>=e[a].size()) d=e[b][j];
		else d=min(e[a][i],e[b][j]);
		res={a,b,c,d};
		sort(all(res));
		chkmin(ans,res);
	}
