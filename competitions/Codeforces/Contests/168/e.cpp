...
int ask(int rt,int l,int r,int x){
	...
	if(l==r) return l;
	int mid=l+r>>1;
	if(x<=st[lc]) return ask(lc,l,mid,x);
	else return ask(rc,mid+1,r,x-st[lc]);
}
...
inline void SOLVE(int Case){
	...
	rep(lv,1,n){
		per(k,K,1){
			int offset=ask(1,1,n,1,lst[k].back());
			int res=ask(1,1,n,k+offset);
			lst[k].pb(res);
			if(res==n) K--;
		}
		for(int x:pos[lv]){
	...
	rep(i,1,q){
		int x,y;
		cin>>x>>y;
		int l=1,r=(int)lst[y].size()-1,res;
		while(l<=r){
			int mid=l+r>>1;
			if(x>lst[y][mid]) l=mid+1;
			else r=(res=mid)-1;
		}
		if(a[x]>=res) cout<<"YES"<<endl;
		else cout<<"NO"<<endl;
	}
}
