/*
 * Author: Austin Jiang
 * Date: 5/18/2023 6:21:21 PM
 * Problem: Range Sorting (Hard Version)
 * Source:
 * Description:
*/
...
void SOLVE(int Case){
	...
	Segtree<int> Kst(1,n,0,[](int x,int y){ return max(x,y);});
	Segtree<int> Xst(1,n,0,[](int x,int y){ return max(x,y);});
	Segtree<int> Yst(1,n,n+1,[](int x,int y){ return min(x,y);});
	VPI q;
	rep(i,1,n){
		k[i]=Kst.ask(1,a[i]);
		Kst.upd(a[i],i);
	...
	int j=1;
	rep(i,1,n){
		while(j<=n&&j<q[i-1].fir){
			Xst.upd(a[j],j);
			j++;
		}
		x[q[i-1].sec]=Xst.ask(a[q[i-1].sec],n);
	}
	per(i,n,1){
		y[i]=Yst.ask(1,a[i]);
		Yst.upd(a[i],i);
