...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		while(!stk.empty()&&a[stk.top()]>a[i]){
			dp[i]=(dp[i]+dp[stk.top()]+sum2[stk.top()])%MOD;
			sum2[i]=(sum2[i]+dp[stk.top()]+sum2[stk.top()])%MOD;
			stk.pop();
		}
		if(stk.empty()){
			dp[i]=(dp[i]+1)%MOD;
			...
			dp[i]=(dp[i]+sum[stk.top()])%MOD;
			sum[i]=(dp[i]+sum[stk.top()])%MOD;
		}
		stk.push(i);
	}
	int ans=0;
