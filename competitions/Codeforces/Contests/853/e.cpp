...
int n,s[N],f[S],flag[S],sum[S],cnt1[S],cnt2[S];
...
void SOLVE(int Case){
	...
	rep(i,1,n) flag[s[i]]++;
	rep(i,1,s[n]) sum[i]=flag[i]+sum[i-1];
	set<int> K1,K2;
	rep(x,1,s[n]){
		if(s[n]%x) K1.insert(s[n]/x);
		else K2.insert(s[n]/x);
	}
	for(int k:K1){
		cnt1[k]=n;
		for(int i=1;i<k&&(i-1)*k+i<=s[n];i++){
			int l=(i-1)*k+i;
			int r=i*k-1;
			cnt1[k]-=sum[min(s[n],r)]-sum[l-1];
		}
	}
	for(auto k:K2){
		for(int i=1;i*k<=s[n];i++){
			cnt2[k]+=flag[k*i];
		}
	}
	rep(x,1,s[n]){
		if(s[n]%x) f[x]=cnt1[s[n]/x];
		else f[x]=cnt2[s[n]/x];
	}
	ll ans=0;
	rep(x,1,s[n]){
