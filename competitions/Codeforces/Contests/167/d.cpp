...
//	}
	...
	rep(i,1,m){
		int j=1;
		if(c[i]<b[j].fir) continue;
//		cout<<"hello"<<endl;
		int step=(max(0ll,c[i]-b[j].fir+1)+b[j].sec-1)/b[j].sec;
//		cout<<i<<" "<<j<<": "<<step<<endl;
		c[i]-=step*b[j].sec;
		chkmax(mxdp,c[i]);
		ans+=step*2;
...
//	cout<<ans<<endl;
	rep(i,0,mxdp){
		...
		if(j==-1) continue;
		int step=(max(0ll,i-b[j].fir+1)+b[j].sec-1)/b[j].sec;
		dp[i]=dp[i-step*b[j].sec]+step*2;
	}
	rep(i,1,m){
		ans+=dp[c[i]];
	}
