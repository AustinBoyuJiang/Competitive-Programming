...
bool check(int lim){
	int res[2]={cnt[0],cnt[1]};
	int b1=a1,b2=a2;
	if(res[0]>lim){
		int k=min(res[0]-lim,b2);
		res[0]-=k;
		b2-=k;
		res[1]+=b1-b2;
	}
	else{
		int k=min(lim-res[0],b1);
		res[0]+=k;
		b1-=k;
		res[1]+=b1-b2;
	}
	
	return min(res[0],res[1])>=lim;
}
...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		if(a[i].fir==0||a[i].sec==0){
			if(a[i].fir>a[i].sec) cnt[0]+=a[i].fir;
			else cnt[1]+=a[i].sec;
		}
		else if(a[i].fir+a[i].sec==0){
			if(a[i].fir>a[i].sec) cnt[0]+=a[i].fir;
			else cnt[1]+=a[i].sec;
		}
		else{
