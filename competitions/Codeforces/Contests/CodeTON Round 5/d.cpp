...
int n,m,ans=INF,dist[2][N],cnt[N],val[N],ord[N];
VPI e[N];

void dijk(int k,int st){
...
void SOLVE(int Case){
	...
	dijk(0,1);
	dijk(1,n);
	rep(i,1,n) chkmin(ans,dist[1][i]+dist[0][i]);
	rep(i,1,n) val[i]=ans-dist[0][i];
	rep(i,1,n){
		chkmax(val[i],0ll);
		chkmin(val[i],ans);
	}
	...
	rep(i,1,n) ord[i]=val[i];
	sort(ord+1,ord+n+1);
	cout<<ans<<" "<<n-1<<endl;
	rep(i,2,n){
		rep(j,1,n){
			if((ord[i]<=val[j]||j==1)&&j!=n) cout<<1;
			else cout<<0;
